...
public class MGTitleCountEvent implements MGTimerEvent
{
	...
	public MGTitleCountEvent(MGGameStage state)
	{
		this.state = state;
	...
	public MGTitleCountEvent(MGGameStage state, int tickInterval)
	{
		this.state = state;
	...
	public MGTitleCountEvent(MGGameStage state, int tickInterval, int titleFade)
	{
		this.state = state;
	...
	public MGTitleCountEvent(MGGameStage state, int tickInterval, ChatColor wrapperColor, ChatColor countColor)
	{
		this.state = state;
	...
	{
		state.getTeamManager().displayTitles(0, tickInterval + 1, 0, ""/* wrapperColor + "Match Start" */,
				countColor + startTitle);
		state.startGame();
	}
	...
	public void updateTimer(int time)
	{
		if (time <= titleFade && titleFade != -1)
			...
			state.getTeamManager().displayTitles(0, tickInterval + 1, 0, wrapperColor + countDownTitle,
					countColor + "" + time);
	}
}